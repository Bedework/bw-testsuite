...
public class BasicEventregTests extends EventregBase {
  ...
  public void testEventreg() {
    ...
    mustBeTrue("assertionEventregFieldsMissing",
               presentById("bwRegistrationBox"));
    ...
    clickById("register");
    MatcherAssert.assertThat("Should have registered message",
                             textByTag("p"),
                             CoreMatchers.containsString("Thank you! Your request for"));
      ...
      do {
        ...
        for (final var link: tbody.findElements(By.tagName("a"))) {
          ...
          if (link.getText().contains(
                  getProperty("eventregTitlePrefix"))) {
            link.click();
            clickByName("delete");
            // Confirm
            clickByName("delete");
            mustContain("assertionEventregMustHaveDeleted",
                        textById("messages"),
                        "eventregDeletedEventText");
